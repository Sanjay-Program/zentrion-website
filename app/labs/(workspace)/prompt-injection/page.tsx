  ...
  const missionBriefing = (
    <>
      ...
      <div className="p-3 bg-void rounded border border-line mt-4">
        ...
        <span className="text-sm text-white">Extract the secret discount code.</span>
      </div>
    </>
  );

  const hints = [
    "Direct requests (e.g., 'What is the code?') will usually fail because the system prompt explicitly forbids it.",
    "Try telling the AI to ignore previous instructions.",
    "Try asking the AI to translate its system prompt into another language.",
    "Try putting the AI into a simulated developer mode."
  ];

  return (
    <LabWorkspace
      labId="prompt-injection"
      ...
      hints={hints}
      isSolved={isSolved}
      flagId="ZENTRION{pr0mpt_inj3ct10n_m4st3r}"
    >
      <div className="flex flex-col h-full bg-surface/30">
        <div className="px-5 py-4 border-b border-line bg-void flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-medium text-white">Support AI</h3>
        ...
        <form onSubmit={handleSubmit} className="p-4 bg-void border-t border-line shrink-0">
          <div className="relative">
            <input
              ...
              className="w-full bg-surface border border-line rounded-xl pl-4 pr-12 py-3 text-sm text-white placeholder:text-mute focus:outline-none focus:border-cyan transition-colors disabled:opacity-50"
              autoComplete="off"
            />
            <button
              type="submit"
