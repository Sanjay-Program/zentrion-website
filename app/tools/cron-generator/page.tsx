      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div className="glass-card rounded-2xl border border-[var(--c-glass-border)] bg-[var(--c-glass-bg)] backdrop-blur-md p-6 sm:p-10 mb-8">
          ...
          <div className="flex flex-col items-center justify-center mb-10">
            ...
            <div className="relative group bg-[rgba(17,17,17,0.05)] dark:bg-[rgba(0,0,0,0.3)] border border-[var(--c-glass-border)] rounded-xl px-8 py-6 w-full text-center">
              <div className="font-mono text-4xl md:text-5xl font-bold text-[rgb(var(--c-accent))] tracking-widest">{cronExpression}</div>
              <button 
                onClick={copyToClipboard}
                className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-[rgba(255,255,255,0.05)] hover:bg-[rgba(255,255,255,0.1)] border border-[var(--c-glass-border)] rounded-lg text-[rgb(var(--c-ink))] opacity-0 group-hover:opacity-100 transition-all"
            ...
            <div className="mt-4 text-[rgb(var(--c-ink))] font-semibold text-lg">{getHumanReadable()}</div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-[rgb(var(--c-ink))]">Minute</label>
              <input 
                type="text" 
                value={minute} 
                onChange={(e) => setMinute(e.target.value)}
                className="w-full px-4 py-3 bg-[var(--c-glass-bg)] border border-[var(--c-glass-border)] rounded-xl text-[rgb(var(--c-ink))] focus:outline-none focus:border-[rgb(var(--c-accent))] focus:ring-1 focus:ring-[rgb(var(--c-accent))] font-mono text-center"
              />
              <span className="text-[10px] text-[rgb(var(--c-mute))] text-center leading-tight">0-59, *, */n</span>
            </div>
            
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-[rgb(var(--c-ink))]">Hour</label>
              <input 
                type="text" 
                value={hour} 
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-[rgb(var(--c-ink))]">Day of Month</label>
              <input 
                type="text" 
                value={day} 
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-[rgb(var(--c-ink))]">Month</label>
              <input 
                type="text" 
                value={month} 
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-[rgb(var(--c-ink))]">Day of Week</label>
              <input 
                type="text" 
                value={weekday} 
        ...
        <div className="glass-card rounded-2xl border border-[var(--c-glass-border)] bg-[var(--c-glass-bg)] backdrop-blur-md p-6 sm:p-8">
          <h3 className="text-[rgb(var(--c-ink))] font-bold text-lg mb-4">Quick Presets</h3>
          <div className="flex flex-wrap gap-3">
            {presets.map((preset, idx) => (
              <button 
                key={idx}
                onClick={() => handlePreset(preset.val)}
                className="px-4 py-2 bg-[rgba(255,255,255,0.05)] hover:bg-[rgba(255,255,255,0.1)] border border-[var(--c-glass-border)] rounded-lg text-sm text-[rgb(var(--c-ink))] transition-all flex flex-col items-start gap-1"
              >
