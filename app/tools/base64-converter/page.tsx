...
export default function Base64ConverterPage() {
  ...
  useEffect(() => {
    if (mode === 'encode') {
      try {
        ...
        // Use btoa, but handle UTF-8 correctly
        const encoded = btoa(encodeURIComponent(plainText).replace(/%([0-9A-F]{2})/g,
  ...
  useEffect(() => {
    if (mode === 'decode') {
      try {
        ...
        // Use atob, handle UTF-8 correctly
  ...
  const copyToClipboard = () => {
    const textToCopy = mode === 'encode' ? base64Text : plainText;
    ...
    <div className="min-h-screen bg-[rgb(var(--c-void))] text-[rgb(var(--c-ink))] pt-32 pb-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="mb-10 text-center sm:text-left">
          <h1 className="text-4xl md:text-5xl font-bold font-display tracking-tight mb-4">Base64 Converter</h1>
          <p className="text-xl text-[rgb(var(--c-mute))]">Instantly encode strings to Base64 or decode Base64 back to plain text.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[500px]">
          ...
          <div className={`glass-card rounded-2xl border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.02)] backdrop-blur-md flex flex-col overflow-hidden transition-all duration-300 ${mode === 'decode' ? 'ring-2 ring-[rgb(var(--c-accent))] shadow-[0_0_30px_rgba(var(--c-accent-rgb),0.1)]' : ''}`}>
            ...
            <textarea
              value={base64Text}
              onChange={(e) => {
                if (mode === 'decode') setBase64Text(e.target.value.replace(/\s/g, ''));
              }}
              readOnly={mode === 'encode'}
              placeholder={mode === 'decode' ? "Paste Base64 string here to decode..." : "Encoded result will appear here..."}
