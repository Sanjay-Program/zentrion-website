          ...
          <div className="glass-card rounded-2xl border border-[var(--c-glass-border)] bg-[var(--c-glass-bg)] backdrop-blur-md flex flex-col overflow-hidden h-full">
            <div className="px-6 py-4 border-b border-[var(--c-glass-border)] bg-[var(--c-glass-bg)] flex justify-between items-center shrink-0">
              ...
              <button
                ...
                className="text-xs px-3 py-1.5 bg-[rgba(255,255,255,0.05)] hover:bg-red-500/20 hover:text-red-400 border border-[var(--c-glass-border)] rounded text-[rgb(var(--c-mute))] transition-colors"
              >
                Clear
              </button>
            </div>
...
strict-transport-security: max-age=31536000; includeSubDomains`}
              className="flex-grow p-6 bg-transparent text-[rgb(var(--c-ink))] font-mono leading-relaxed focus:outline-none resize-none custom-scrollbar whitespace-pre"
              spellCheck="false"
            />
          </div>
          ...
          <div className="glass-card rounded-2xl border border-[var(--c-glass-border)] bg-[var(--c-glass-bg)] backdrop-blur-md flex flex-col overflow-hidden h-full">
            <div className="px-6 py-4 border-b border-[var(--c-glass-border)] bg-[var(--c-glass-bg)] flex justify-between items-center shrink-0 gap-4">
              <h3 className="text-[rgb(var(--c-ink))] font-semibold flex items-center gap-2 shrink-0">
                ...
                <div className="p-4 space-y-4">
                  {statusLine && (
                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                      ...
                      <div className="font-mono text-[rgb(var(--c-ink))] break-all">{statusLine}</div>
                    </div>
                  )}

                  {parsedHeaders.length > 0 && (
                    <table className="w-full text-left border-collapse">
                      <thead className="border-b border-[var(--c-glass-border)]">
                        <tr>
                      ...
                      <tbody className="divide-y divide-[var(--c-glass-border)]">
                        {filteredHeaders.map((h, i) => (
                          <tr key={i} className={`hover:bg-[var(--c-glass-bg)] transition-colors ${h.isSecurity ? 'bg-[rgb(var(--c-accent))]/5' : ''}`}>
                            <td className="py-3 px-4 font-mono text-sm w-1/3 align-top break-all">
                              <span className={h.isSecurity ? 'text-[rgb(var(--c-accent))] font-bold flex items-center gap-1.5' : 'text-[rgb(var(--c-ink))]'}>
                            ...
                            <td className="py-3 px-4 font-mono text-sm text-[rgb(var(--c-mute))] align-top break-all">
                              {h.value}
                            </td>
                          </tr>
                        ))}
